...
from survae.transforms import AdditiveCouplingBijection, AffineCouplingBijection, ActNormBijection, ActNormBijection1d, Reverse, Shuffle, PermuteAxes, Reshape, ScalarAffineBijection, StochasticPermutation
from survae.nn.layers import ElementwiseParams1d, scale_fn
from nets import DenseTransformer
...
parser.add_argument('--stochperm', type=eval, default=True)
...
# Eval params
parser.add_argument('--iwbo_k', type=int, default=10)
parser.add_argument('--iwbo_kbs', type=int, default=None)
...
run_name = '{}_perm_flow_{}'.format(args.dataset, args.name)
if args.train:
...
def lenwise(transforms):
    net = nn.Sequential(DenseTransformer(d_input=D, d_output=P*D, d_model=args.d_model, nhead=args.nhead,
                                         num_layers=args.num_layers, dim_feedforward=4*args.d_model,
                                         dropout=args.dropout, activation=args.activation,
                                         checkpoint_blocks=args.checkpoint_blocks),
                        ElementwiseParams1d(P))
...
def evaluate_iwbo(model, eval_loader):
    ...
    with torch.no_grad():
        ...
        for i, x in enumerate(eval_loader):
            x = x.to(args.device)
            loss_sum += - iwbo_nats(model, x, k=args.iwbo_k, kbs=args.iwbo_kbs).cpu().item() * len(x) / 50
            count += len(x)
            print('IWBO({}), Iter: {}/{}, PPLL: {:.3f}'.format(args.iwbo_k, i+1, len(eval_loader), loss_sum/count), end='\r')
...
# Save log-likelihood
with open('results/{}_test_loglik{}.txt'.format(run_name, args.iwbo_k if args.iwbo_k else ''), 'w') as f:
